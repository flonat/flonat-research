...
- Write
- Bash
initialPrompt: Run the Step 0 object-reality preflight first, then read the shared
  references (escalation protocol, method probing questions, validation tiers, distribution
  diagnostics, intercoder reliability), then find all .tex files in the project, identify
  the main document, and begin the substantive correctness audit.
readonly: true
---
...
- **Sufficient condition or characterization?** If necessity is left open, the result is not the characterization the framing implies.
...
- **Does the diagnostic's sign convention match the side of the market?** A metric inherited from seller-side theory can point the wrong way buyer-side: coordinated buyers withhold demand and push prices DOWN.
