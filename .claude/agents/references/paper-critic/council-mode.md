...
>
> **This document is addressed to the main session, not the sub-agent.** When council mode is triggered (user says "council mode", "council review", or "thorough quality check"), the main session orchestrates using the `llm-council` Python package (or `cli-council`) — it does NOT launch a single paper-critic agent.
