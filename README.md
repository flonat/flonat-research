...
| [`council-cli`](https://github.com/flonat/council-cli) | Clone and use `uv` | Multi-model council via installed local CLI tools |
...
|-----------|-------|-------------|
| **Skills** | 109 | Portable workflows for common tasks (`proofread`, `latex`, `literature`, etc.) |
...
| **Scholarly search** | — | `scholarly` CLI for shell clients plus an optional Claude MCP adapter — [setup guide](docs/bibliography-setup.md) |
...
109 skills available. Key examples: `proofread`, `latex`, `literature`, `bib-validate`, `init-project-research`, `pre-submission-report`, and more. Use `/name` in Claude, `$name` in Codex, or natural language; see [`docs/availability.md`](docs/availability.md).
<!-- SKILLS-SUMMARY:END -->
...
**Research Vault** — Optional Obsidian-style markdown vault for tasks,
pipeline, submissions, venues, people, and themes. Claude can use an MCP
adapter; shell-capable Claude/Codex workflows use `taskflow-cli`.

**Scholarly search** — The `scholarly` CLI provides multi-source search for
shell-capable clients; Claude Desktop can use the optional MCP adapter. A
separate Paperpile/RefPile setup may provide personal-library search. See
...
├── skills/                      # 109 client-neutral workflows
